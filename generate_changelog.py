...
def _write_minor_analysis(
    ...
    if do_changelog:
        major_label = minor_dir.parent.name
        content_parts = [
            f'# FortiGate {minor_dir.name} — Analysis', '',
            f'[← {major_label} Index](../INDEX.md)',
            '',
...
def _write_changelog_index(root: Path, version_groups: list[tuple[str, list[Path]]]) -> None:
    ...
    def _major_section(major_label: str, minor_dirs: list[Path], prefix: str, include_matrices: bool = False, link_heading: bool = False) -> list[str]:
        """Build the lines for one major-version section. prefix is prepended to all paths."""
        sec: list[str] = []
        heading = f"[FortiOS {major_label}]({major_label}/INDEX.md)" if link_heading else f"FortiOS {major_label}"
    ...
    for major_label, minor_dirs in reversed(version_groups):
        ...
        root_lines.extend(_major_section(major_label, minor_dirs, prefix=f"{major_label}/", include_matrices=True, link_heading=True))
        root_lines.append("---")
        root_lines.append("")
