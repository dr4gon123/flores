...
Two levels of `INDEX.md` provide navigable entry points into the repository:

**[INDEX.md](INDEX.md)** — root index covering all major versions. For each major version it lists:
- **Changelog** — links to per-minor `CHANGELOG.md`
- **Consolidated Fields** — links to `fields/{log_type}_fields.csv`
- **ECS** — links to `ECS/{log_type}_ecs.csv`
- **Field Occurrence Matrix** — per-minor links to `traffic_matrix.csv · event_matrix.csv · utm_matrix.csv`

**`{major}/INDEX.md`** (e.g. [7.6/INDEX.md](7.6/INDEX.md)) — same sections scoped to one major version, with links relative to that folder.

Both files are auto-generated by `python3 generate_changelog.py --index`.
